---
name: new-experiment
description: Scaffold and run a new experiment in this repo. Creates experiments/<name>/ with src/, tests (recorded HTTP), scripts/ that emit results/*.csv, blog.md, a generated index.html that publishes it, and log.md. Use whenever the user wants to start, set up, or create a new experiment, probe, benchmark, or investigation.
---

# New experiment

Each experiment is its own pnpm workspace under `experiments/<name>/`. The code
is the _means_; the writeup, the raw measurements and the decision trail are the
_output_. An experiment is not done until all of them exist and agree.

## This repo is public

Everything committed here is published. Nothing proprietary goes in: not in
code, prompts, test fixtures, recordings, CSVs, logs, or the blog.

- **No company or customer data.** No internal prompts, product code, customer
  conversations, user IDs, internal URLs, or anything copied from private repos
  or dashboards. When an experiment needs realistic input, generate synthetic
  data (seeded, via a script) or use public datasets, and say so in the blog.
- **No credentials or account identifiers.** Keys live only in the gitignored
  `.env`. `@experiments/test` masks auth headers, org/project/workspace IDs, rate-limit headers and cookies
  before writing recordings, but check any new provider's response headers
  for identifiers and add them to `SENSITIVE_HEADERS` in
  `packages/test/src/record.ts` before committing its recordings.
- **Review before committing.** Grep new recordings and results for
  `organization`, `project`, `workspace`, `ratelimit`, `cookie`, `sk-`, `AIza`, emails and internal
  hostnames. If something private slipped in, delete it and re-record rather
  than editing the HAR.
- **Write for outsiders.** The blog and log shouldn't reference internal
  projects, roadmaps, or why the company cares. Frame the question on its own
  merits.

If the user asks for something that would require private data, say so and
propose a synthetic or public substitute.

## 1. Pin down the question

Before creating files, get a one-sentence question the experiment answers
(e.g. "Does changing `toolConfig.mode` between turns break Gemini's implicit
cache?"). If the user's ask is vague, propose one and confirm. Pick a
`snake_case` name from it (`config_cache`, `gemini_local_tokens`).

If the question is really a follow-up to an existing experiment, check
`experiments/` first. Spin out a new one only when it's a different question;
otherwise add to the existing log.

## 2. Scaffold

```
experiments/<name>/
  package.json
  tsconfig.json
  vitest.config.ts
  src/                 # reusable logic, imported by scripts and tests
    *.test.ts          # tests, co-located with the code they cover
  scripts/             # one script per results CSV, plus build-report.ts
  results/             # *.csv, only ever written by scripts/
  blog.md              # the write-up, in Markdown
  index.html           # generated by scripts/build-report.ts from blog.md, never hand-edited
  log.md
```

### `package.json`

```json
{
  "name": "@experiments/<name>",
  "version": "0.0.0",
  "private": true,
  "type": "module",
  "scripts": {
    "typecheck": "tsc --noEmit",
    "test": "vitest run",
    "report": "tsx scripts/build-report.ts"
  }
}
```

Add one script entry per run script (e.g. `"bench": "tsx scripts/run-bench.ts"`).

Then add deps with pnpm, never by hand-editing versions:

```sh
pnpm --filter @experiments/<name> add '@experiments/shared@workspace:*' ai
pnpm --filter @experiments/<name> add -D '@experiments/test@workspace:*'
```

Shared dev tooling (typescript, tsx, eslint, prettier, vitest, `@types/node`)
lives at the repo root. Don't reinstall it per workspace.

### `tsconfig.json`

```json
{
  "extends": "../../tsconfig.base.json",
  "compilerOptions": { "outDir": "dist" },
  "include": ["src/**/*", "scripts/**/*", "vitest.config.ts"]
}
```

### `vitest.config.ts`

```ts
import { defineConfig } from "vitest/config";

export default defineConfig({
  test: { include: ["src/**/*.test.ts"], environment: "node" },
});
```

### `log.md`

```md
# <name> log

Chronological notes: what was tried, what was found.

## YYYY-MM-DD: <the question, as first posed>

<why this experiment exists, and the plan>
```

### `blog.md`

Start with a title phrased as the question and a `## Method` / `## Findings` /
`## Takeaways` skeleton with a sentence or two of intent under each. It gets
filled in as results arrive, not at the end.

Use real dates (today's date from context), never "today" or "yesterday".

## 3. Tests: the proof of work

An experiment is a **reproducible study**. Someone who has never seen the
conversation that produced it must be able to verify every claim in `blog.md`
by running the tests, without an agent and without trusting the CSVs. The
tests and their committed recordings are the proof of work.

Two kinds of test, both co-located in `src/`:

- **Harness tests** pin down the machinery: helpers, request shapes, parsing
  of provider responses.
- **Claim tests** (`src/claims/*.test.ts`) reproduce the findings. **Every
  claim in `blog.md` has a claim test**, named as the claim in plain words
  (`"an edit to the system prompt reuses nothing"`), which runs the same live
  procedure the scripts use and asserts the outcome the blog states. One
  test may cover a family (`it.each` over all 30 pairs), but no claim goes
  untested. The blog cites the test file next to each claim. The same goes
  for claims of the form "X has no effect" and for tests that rule out an
  alternative explanation (see § 6).

Import `describe` / `it` / `expect` from `@experiments/test`, not `vitest`.
Every `it` is wrapped in Polly recording, so LLM calls are recorded once and
replayed after:

```sh
pnpm --filter @experiments/<name> test              # replay: verify every claim offline
RECORD=true pnpm --filter @experiments/<name> test  # re-verify every claim live, re-record
RECORD=new  pnpm --filter @experiments/<name> test  # record only tests with no recording yet
```

Rules that keep this honest:

- **Fresh on record, identical on replay.** Anything that must differ
  between live runs (nonces, cache keys, random seeds that shape a request)
  comes from `recordedValue(key, make)` in `@experiments/test`: generated
  when recording, stored next to the HARs, read back on replay. A live
  re-run must never be able to pass by hitting state left by the previous
  recording.
- **Assert the claim, not the recorded numbers.** Assertions state what the
  blog states ("cached is 0", "input tokens unchanged", "within 2 tokens of
  a 119-token tail"), so a live re-run can pass or fail on its merits.
- **Commit the recordings** under `test/__recordings__/` together with
  `values.json`. If the provider changes behaviour, a live re-run fails the
  specific claim that no longer holds; update the blog, not the assertion.
- Don't mark recorded tests `.concurrent`; split long suites across files
  (vitest runs files in parallel).

Claim tests show that each claim reproduces; the scripts and CSVs (§ 4)
show how consistently, with n trials per condition. A claim needs both.

## 4. Scripts and results

- Every CSV in `results/` has exactly one script in `scripts/` that produces it.
  CSVs are never hand-written or hand-edited.
- **Every run script can be run N more times to increase certainty.** It takes
  `RUNS=N` (trials per condition, default small) and **appends** N new trials
  per condition to its CSV rather than overwriting, so evidence accumulates
  across invocations. `FRESH=true` wipes the CSV first. If the columns change,
  the script refuses to append and asks for `FRESH=true`.
- One row per trial. Include enough columns to re-derive every number
  quoted in the blog (model, variant, seed, raw token counts, timestamps),
  not just the summary, plus a `batch` id (the invocation's start time) and
  `run` index so trials from different invocations stay distinguishable.
- Each trial must be independent of every other trial (fresh seed/nonce,
  no shared state), so trials from separate invocations can be pooled.
- Analysis and the report always aggregate over **all** rows in the CSV and
  show the trial count (`n`) and how consistent the trials were next to every
  number, so it is obvious where more runs are needed.
- Pilot with `RUNS=1` on a subset (e.g. a `VARIANTS=` filter) before a full
  run, and log the pilot's cost and surprises.
- Put a header comment at the top of each script stating the question, the
  per-trial procedure, and what outcome would confirm or refute the hypothesis.
- Make knobs env-overridable (`MODEL`, `RUNS`, etc.) with sensible defaults.
- Keep reusable logic in `src/` so tests can cover it.

## 5. `index.html`

`index.html` is the experiment’s published page, so `experiments/<name>/` is
the URL people read. `scripts/build-report.ts` reads `blog.md` and
`results/*.csv` and writes it as a single self-contained file: **the whole
blog rendered to HTML**, with tables and charts built from the CSVs in place
of (or alongside) the markdown tables. `blog.md` is its source, not a second
page: the site excludes it, so everything a reader should see goes in
`blog.md`, and charts go where the blog marks them with an HTML comment
(e.g. `<!-- report:static -->`), which renders as nothing in the Markdown.

- Generated, never hand-edited, so it can't drift from the data. Re-run
  `pnpm --filter @experiments/<name> report` after any results or blog change.
- Self-contained: inline CSS and data. Libraries only from a CDN
  (cdn.jsdelivr.net / cdnjs), no local asset files.
- Works in light and dark mode and at phone width.
- Load the `dataviz` skill before writing chart code.

## 6. While running the experiment

- **Append to `log.md` immediately** after each attempt: what was tried, what
  happened (with numbers), what was learned. Dead ends, surprises and bugs in
  the harness count. Don't batch it up for the end.
- Update `blog.md` as findings firm up. It draws from the log and the CSVs,
  cites the script and CSV behind each claim, and is written for someone who
  never saw the log.
- **State every claim at the scope of the setup that produced it.** A
  synthetic setup is usually a special case: a request whose history was
  never sent turn by turn, one model, one size. Before a claim says what
  happens "in a conversation" or "on this API", test the realistic case
  (a real multi-turn thread, the way clients actually call the API). Lead
  with the realistic result, and label the synthetic one as the edge case
  it is. A strong negative ("invalidates everything", "never reused") needs
  the realistic test most of all.
- **Rule out the obvious alternative explanation before stating a
  conclusion.** If a result could come from a threshold or a confound
  (a minimum size, a rate limit, routing, a too-small prompt, one lucky
  seed), run the decisive test that would make it go away: repeat at a much
  larger size, a different seed, a different key. Put that test in a script
  and the CSV like any other result, and say in the blog what it ruled out.
- Rebuild `index.html` whenever results or the blog change.

## 6b. Publishing

The repo is published with GitHub Pages (see the README). The site's home
page is generated by `pnpm pages:index` in the Pages workflow and lists every
experiment folder with a `blog.md` and a `log.md`, so there is nothing to
register. It reads three things, so keep them in shape:

- the blog's `# ` title (the question),
- the blog's **first paragraph** after the title, used as the summary, so it
  should say what the experiment asks in two or three sentences,
- the `## YYYY-MM-DD: ...` headings in `log.md`, whose first and last dates
  are shown as when the experiment started and was last updated.

Run `pnpm pages:index` and open the root `index.html` to check the entry.
Markdown needs no front matter (Pages
takes the title from the first heading), and `build-report.ts` renders
`blog.md` as-is, so front matter would show up in the report. Don't put
Liquid template syntax (two opening curly braces, or a curly brace and a
percent sign) in Markdown: Jekyll would try to evaluate it and the Pages
build fails.

## 7. Done means

- [ ] `blog.md` answers the question, with Method, Findings and Takeaways, and
      cites `scripts/*` and `results/*` for every result.
- [ ] Every claim in `blog.md` has a claim test in `src/claims/` that
      reproduces it, cited next to the claim, with recordings committed.
      `blog.md` has a "Verify it yourself" section with the two commands.
- [ ] A clean replay (`pnpm test`, no network) passes, and the latest
      `RECORD=true` run passed live.
- [ ] `log.md` has dated entries covering every run, including failures.
- [ ] Every `results/*.csv` is reproducible from its script, and re-running
      it with `RUNS=N` adds N more trials per condition.
- [ ] `index.html` rebuilt from current data and blog, and the experiment's
      entry on the home page (`pnpm pages:index`) has the right title,
      summary and dates.
- [ ] `pnpm --filter @experiments/<name> test` and `typecheck` pass, and
      `pnpm lint` is clean.

Report back to the user with the headline finding and the path to
`index.html`.
