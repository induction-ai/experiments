...
Browse them at **https://induction-ai.github.io/experiments/**. The home page is generated from the experiments themselves (see [Publishing](#publishing-github-pages)), so there is no list to maintain here. Every claim in a write-up has a recorded test that reproduces it (`pnpm --filter @experiments/<name> test`).
...
- **`index.html`** — the published page, generated by `scripts/build-report.ts`: `blog.md` rendered in full, with charts and tables from the CSVs. Never hand-edited.
...
The site is built by `.github/workflows/pages.yml` on every push to `main` (Settings → Pages → Source: **GitHub Actions**). The workflow runs `pnpm pages:index`, then the same Jekyll build GitHub uses for branch deploys.

- **Home page:** `pnpm pages:index` (`packages/site`) writes `index.html` at the repo root, listing every experiment folder that has a `blog.md` and a `log.md`, most recently updated first. Each entry takes its title from the blog’s `# ` heading, its summary from the blog’s first paragraph, and its dates from the first and last `## YYYY-MM-DD` headings in `log.md`. The file is gitignored; run the command locally to preview it. On github.com this README stays the front page.
- **Experiment pages:** each experiment’s generated `index.html` is its page, so `experiments/<name>/` is the URL to share. `blog.md` is excluded from the site because that page already renders it.
- `_config.yml` keeps code and HTTP recordings out of the site. Markdown needs no front matter, since Pages derives each page title from its first heading and turns links to `.md` files into links to the rendered pages.
- Don’t put Liquid template syntax (two opening curly braces, or an opening curly brace followed by a percent sign) in Markdown: Jekyll would try to evaluate it and the build fails. Wrap such content in a Liquid `raw` block.
